...
class RoleBaseDAO{
	...
	function insert($role) {
		...
		try{
			...
			$query = "INSERT INTO role " . $db_insert . " VALUES ( ?, ?)";
			...
			if($stmt->execute()){
				log_debug(get_class($this),"Insert Table role " . $role->role_id. " Success.");
			}else{
	...
	function update($role) {
		...
		try{
			...
			if($stmt->execute()){
				log_debug(get_class($this),"Update Table role " . $role->role_id. " Success.");
			}else{
	...
	function delete($role) {
		...
		try{
			...
			if($stmt->execute()){
				log_debug(get_class($this),"Delete Table role " . $role->role_id. " Success.");
			}else{
	...
	function findbyPK($role) {
		...
		try{
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find By PK role " . $role->role_id. " Success.");
			}else{
	...
	function findAll() {
		...
		try{
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find All role Success.");
			}else{
	...
	function findbyRoleId($role) {
		...
		try{
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find RoleId role " . $role->role_id. " Success.");
			}else{
	...
	function findbyRoleName($role) {
		...
		try{
			...
			$query = "SELECT * FROM role WHERE role_name = ?";
			log_debug(get_class($this),$query);
			if (!($stmt = $DB_CONNECT->prepare($query))) {
				log_debug(get_class($this),"Prepare failed: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find RoleName role " . $role->role_id. " Success.");
			}else{
