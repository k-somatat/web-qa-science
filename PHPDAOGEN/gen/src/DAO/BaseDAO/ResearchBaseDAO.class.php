...
class ResearchBaseDAO{
	...
	function update($research) {
		...
		try{
			...
			$query = "UPDATE  research SET ". $db_update;
			log_debug(get_class($this),$query);
			if (!($stmt = $DB_CONNECT->prepare($query))) {
				log_debug(get_class($this),"Prepare failed: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
	...
	function findbyPK($research_id) {
		...
		try{
			...
			if($stmt->execute()){
				$row = $stmt->bind_result($research_id, $research_name, $research_author, $research_institution, $research_location, $research_detail, $research_date, $research_date_end, $research_budget, $research_document, $user_id, $research_type_id);
				while($stmt->fetch()){
