...
class ConferenceBaseDAO{
	...
	function insert($conference) {
		...
		$db_insert = "(conference_id,
					...
					conference_document,
					conference_time_create,
					conference_time_update,
					user_id)";
		try{
			if($DB_CONNECT){
			...
			$stmt->bind_param("issssssdssssi",
					...
					$conference->conference_time_create,
					$conference->conference_time_update,
					$conference->user_id
				);
...
//		log_debug(get_class($this),"Input : [" . $conference . "].");
	...
	function findbyPK($conference_id) {
		...
		try{
			...
			if($stmt->execute()){
				$row = $stmt->bind_result($conference_id, $conference_name, $conference_institution, $conference_date, $conference_date_end, $conference_location, $conference_topic, $conference_budget, $conference_tech_name, $conference_document, $conference_time_create, $conference_time_update, $user_id);
				while($stmt->fetch()){
					...
					$vo['conference_topic'][] = $conference_topic;
					...
					$vo['conference_time_create'][] = $conference_time_create;
					$vo['conference_time_update'][] = $conference_time_update;
					$vo['user_id'][] = $user_id;
	...
	function findbyConferenceTopic($conference) {
		log_debug(get_class($this),"Input : [" . $conference . "].");
		$DB_CONNECT = new MySQLi(DB_HOST,DB_USERNAME,DB_PASSWORD,DB_NAME);
		$DB_CONNECT->set_charset('utf8');
		try{
			...
			$query = "SELECT * FROM conference WHERE conference_topic = ?";
			log_debug(get_class($this),$query);
			if (!($stmt = $DB_CONNECT->prepare($query))) {
				log_debug(get_class($this),"Prepare failed: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find ConferenceTopic conference " . $conference->conference_id. " Success.");
			}else{
				log_debug(get_class($this),"Find ConferenceTopic Error: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
		}catch(Exception $e){
			log_debug(get_class($this),"Error: " . $DB_CONNECT->error);
		}
	...
	function findbyConferenceTimeCreate($conference) {
		log_debug(get_class($this),"Input : [" . $conference . "].");
		$DB_CONNECT = new MySQLi(DB_HOST,DB_USERNAME,DB_PASSWORD,DB_NAME);
		$DB_CONNECT->set_charset('utf8');
		try{
			...
			$query = "SELECT * FROM conference WHERE conference_time_create = ?";
			log_debug(get_class($this),$query);
			if (!($stmt = $DB_CONNECT->prepare($query))) {
				log_debug(get_class($this),"Prepare failed: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find ConferenceTimeCreate conference " . $conference->conference_id. " Success.");
			}else{
				log_debug(get_class($this),"Find ConferenceTimeCreate Error: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
		}catch(Exception $e){
			log_debug(get_class($this),"Error: " . $DB_CONNECT->error);
		}
	...
	function findbyConferenceTimeUpdate($conference) {
		log_debug(get_class($this),"Input : [" . $conference . "].");
		$DB_CONNECT = new MySQLi(DB_HOST,DB_USERNAME,DB_PASSWORD,DB_NAME);
		$DB_CONNECT->set_charset('utf8');
		try{
			...
			$query = "SELECT * FROM conference WHERE conference_time_update = ?";
			log_debug(get_class($this),$query);
			if (!($stmt = $DB_CONNECT->prepare($query))) {
				log_debug(get_class($this),"Prepare failed: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
			...
			if($stmt->execute()){
				...
				return $vo;
				log_debug(get_class($this),"Find ConferenceTimeUpdate conference " . $conference->conference_id. " Success.");
			}else{
				log_debug(get_class($this),"Find ConferenceTimeUpdate Error: (" . $DB_CONNECT->errno . ") " . $DB_CONNECT->error);
			}
		}catch(Exception $e){
			log_debug(get_class($this),"Error: " . $DB_CONNECT->error);
		}
