...
<div id="page-wrapper" class="page-wrapper">
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <ul class="nav nav-pills">
                <li class="active"><a href="#admin_conference">ข้อมูลสรุปผลการอบรม/สัมมนา/ประชุมวิชาการทั้งหมด</a></li>
                <li><a href="admin_conference_form.php">เพิ่มข้อมูลสรุปผลการอบรม/สัมมนา/ประชุมวิชาการ</a></li>
            </ul>
        </div>
    </div>
    <!-- /.row -->
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="admin_conference">
            ...
            <table class=" table-bordered ">
                ...
                $conferencesUserId = $conferenceDao->findAll();
                $_SESSION['conference'] = $conferencesUserId;

                for($Index = 0; $Index < count($conferencesUserId['conference_id']); $Index++){
                    $user = $userDAO->findbyUserId($conferencesUserId['user_id'][$Index]);
                    $splitUserName = explode(' ',$user['user_first_name'][0]);
                    $_SESSION['conference']['userName'][$Index] = $splitUserName[0].$splitUserName[1].' '.$user['user_last_name'][0];
                }
...
//                    if ($conference['user_id'][$index] == $_SESSION["USER"]["user_id"][0]) {
                        ...
                        echo
                            "<td>" .
                            "<button class='btn btn-large btn-info btn-primary' onclick=window.location.href='admin_conference_form.php?cid=" . $conference['conference_id'][$index] . "'>
    ...
    function confirm_delete(cid) {
        bootbox.confirm("ยืนยันการลบข้อมูล !", function (result) {
            if (result) {
                ...
                window.location = 'sqlfunction.php?method=admin_delete_conference&cid=' + cid;
            } else {
...
<script>
    function export_xls() {
        window.location = 'src/function/export_conference_admin_xls.php';
    }
</script>
